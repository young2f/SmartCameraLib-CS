    ...
    public class DHBase64
    {
        ...
        private LoginResult Login(CameraEntity camera)
        {
            lock (lockObj)
            {
                ...
                loginId = NETClient64.Login(camera.IpAddress, port, camera.UserName, camera.Password, EM_LOGIN_SPAC_CAP_TYPE.TCP, IntPtr.Zero, ref m_DeviceInfo);

                if (IntPtr.Zero == loginId)
                {
                    result.message = camera.IpAddress + " 登录失败: " + NETClient64.GetLastError();
        ...
        public void StopPlay()
        {
            ...
            {
                if (!NETClient64.StopRealPlay(playId))
                {
