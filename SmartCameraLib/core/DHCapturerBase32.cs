...
{
    public class DHCapturerBase32 : DHBase32
    {
        ...
        public override LoginResult LoginSuccess(CameraEntity entity, LoginResult result)
        {
            ...
            bool ret = NETClient32.SnapPictureEx(m_LoginID, asyncSnap, IntPtr.Zero);
            if (!ret)
            {
                result.isOk = false;
                Logout(entity.IpAddress);
                result.message = "图像采集错误码：" + NETClient32.GetLastError();
                return result;
            }
