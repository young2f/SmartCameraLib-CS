...
{
    public class HKBase64
    {
        public HKCHCNetSDK64.REALDATACALLBACK RealData64 = null;
        ...
        private LoginResult Login(CameraEntity camera)
        {
            lock (lockObj)
            {
                ...
                HKCHCNetSDK64.NET_DVR_DEVICEINFO_V30 DeviceInfo = new HKCHCNetSDK64.NET_DVR_DEVICEINFO_V30();

                //登录设备 Login the device

                Int32 loginId = HKCHCNetSDK64.NET_DVR_Login_V30(camera.IpAddress, camera.Port, camera.UserName, camera.Password, ref DeviceInfo);

                if (loginId < 0)
                {
                    result.message = "登录失败:" + HKCHCNetSDK64.NET_DVR_GetLastError();
        ...
        public void StopPlay()
        {
            ...
            if (playId >= 0)
            {
                if (!HKCHCNetSDK64.NET_DVR_StopRealPlay(playId))
                {
        ...
        public void Logout(Int32 loginId)
        {
            if (loginId >= 0)
            {
                if (!HKCHCNetSDK64.NET_DVR_Logout(loginId))
                {
